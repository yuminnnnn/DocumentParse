...
class VlmPipeline(PaginatedPipeline):
    ...
    def _get_flat_text_from_doc_item(self, doc_item: DocItem) -> str:
        """
        Recursively extracts all text from a DocItem and its descendants.
        Handles DocItems that may not have an 'iterate_items' method (e.g., TitleItem).
        """
        all_texts = []
        
        # If the item is a TextItem, directly get its text
        if isinstance(doc_item, TextItem):
            all_texts.append(doc_item.text)
        # If the item has an 'iterate_items' method, traverse its children
        elif hasattr(doc_item, 'iterate_items'):
            for sub_item, _ in doc_item.iterate_items(with_groups=True, traverse_pictures=True):
                if isinstance(sub_item, TextItem):
                    all_texts.append(sub_item.text)
                # Recursively call for other DocItem types that might contain text
                elif isinstance(sub_item, DocItem) and hasattr(sub_item, 'text'):
                    all_texts.append(sub_item.text)
        # For other DocItem types that might have a 'text' attribute but no 'iterate_items'
        elif hasattr(doc_item, 'text'):
            all_texts.append(doc_item.text)
